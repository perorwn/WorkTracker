...
측정 프로그램이 실행 중일 때 웹의 `창모드` 버튼, 트레이 메뉴의 `창모드 열기`, 또는 `Ctrl+Shift+F1`~`F4`를 누르면 Microsoft Edge 앱 창으로 열립니다. 주소 표시줄 없이 항상 위 Windows 창으로 동작하며, 마지막 창 크기와 위치는 다음 실행에도 유지됩니다. 창이 열린 상태에서는 `Ctrl+Shift+-`로 현재 기능의 왼쪽 버튼을, `Ctrl+Shift++`로 오른쪽 버튼을 실행할 수 있습니다.
