...
import { fetchWorkRows, type WorkRow } from "@/lib/supabase"
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-5 py-10 sm:px-8 sm:py-14">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground" aria-live="polite">
        <span suppressHydrationWarning>{loading ? "기록 불러오는 중…" : error ? error : updated ? `최근 갱신 ${updated.toLocaleTimeString()} · 1분마다 갱신${rows.length ? "" : " · 저장된 기록이 없습니다"}` : ""}</span>
